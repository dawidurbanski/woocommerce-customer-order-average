...
/**
 * WC_Report_Customer_List modified.
 *
 * @author      WooThemes
 * @category    Admin
...
class WC_Report_Customer_List extends WP_List_Table {
	...
	public function column_default( $user, $column_name ) {
		switch ( $column_name ) {
			...
			case 'spent_average':
				$average = wc_get_customer_total_spent( $user->ID ) / wc_get_customer_order_count( $user->ID );

				$average_meta = get_user_meta( $user->ID, 'order_average', true );
				if ($average_meta) {
					$average = $average_meta;
				}

				return wc_price( $average );
	...
	public function get_columns() {
		$columns = array(
			...
			'spent_average' => __( 'Order average', 'woocommerce' ),
	...
	public function get_sortable_columns() {
		$columns = array(
			'spent_average' => ['order_average', true],
		);
	...
	public function order_by_last_name( $query ) {
		...
		$order = ! empty( $_REQUEST['order'] ) ? stripslashes( $_REQUEST['order'] ) : 'asc';
		...
		if ( ! empty( $_REQUEST['order'] ) ) {
			$query->query_from   .= " LEFT JOIN {$wpdb->usermeta} as meta2 ON ({$wpdb->users}.ID = meta2.user_id) ";
			$query->query_where  .= " AND meta2.meta_key = 'order_average' ";
